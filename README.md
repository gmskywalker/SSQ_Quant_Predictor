...
- 重要提醒：**有不会的就去问AI**
- 把项目网址链接复制给大模型他会一步步教你，哪怕是0基础小白也能完美运行（推荐使用Gemini，专业在线而且情绪价值给的是真足，国内大模型访问不了github链接效果很差，Gemini给个链接甚至可以读取里面所有文档给出完美的新手教程）
...
流水线运行结束后，会为您生成 5 组不同策略维度的号码矩阵：
1. **EMA 平滑追热**：捕捉近期频繁出现的短线热号。
2. **破冰冷号狙击**：专门埋伏长期未出、随时可能均值回归的极冷号。
3. **Apriori 关联阵**：基于历史大数据，寻找“如果出A，大概率带出B”的连带号码。
4. **物理步长推演**：根据号码的跨度和移动步长计算物理惯性。
5. **👑 宏观黄金防守（🏆 最佳策略）**：**这是平衡了上述所有 AI 算力与统计学的最终王牌底仓！**

> **🎯 终极押仓**：如果您预算有限，或者不想做选择题，只想选一注购买：**请直接无脑跟注第 5 组（最后一组）号码！** 它是当前算法能给出的最高概率综合解。
...
> 小贴士：如果流水线第一步配置环境总是失败，请自行查找虚拟环境安装位置右键在记事本中编辑，可以问AI，实在搞不定的话就不要用这个一键启动了，按下面的自己逐步运行也不麻烦
...
4. 本项目完全开源，代码作者不收取任何费用，当然，要是有朝一日有人真用这玩意中了500万想分我一杯羹我当然会欣然接受的：）
